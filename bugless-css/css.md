...
* program header를 잘 보면, curriculum의 header와 **패턴이 같음**을 알 수 있다. 그래서 **공통으로 적용**하기 위해 section이라는 class로 정의했다. 
...
* 데스크탑 버전에서는 10 column만을 사용했는데, left로 치우쳐있었다. 이를 해결하기 위해, **row 옆에 justify-content-center**를 써주었다. 이는 부트스트랩이 row를 만들기 위해 사용하고 있는 속성이 flex이기 때문에 가능하다.  
