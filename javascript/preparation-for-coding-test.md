...
* 2n개의 \[x1, x2, ... xn, y1, y2, ... yn\]라는 배열이 주어졌을 때, \[x1, y1, x2, y2, ..., xn, yn\]이라는 배열을 리턴하도록 하는 함수 작성. 
* **splice**는 배열의 기존 요소를 삭제  또는 교체하거나 새 요소를 추가하여 **배열의 내용을 변경** 
...
var shuffle = function(nums, n) {
    ...
    var nums2 = []; 
    var final = []; 
    nums1 = nums.slice(0, n);
    nums2 = nums.slice(n, 2 * n);
    nums1.forEach((num, idx) => {
        final.push(num); 
        nums2.forEach((num2, idx2) => {
            if(idx == idx2){
                final.push(num2); 
            }
        });
